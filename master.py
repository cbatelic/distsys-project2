...
async def get_function(req):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            for index,element in dataDict.items():
                tasks.append(asyncio.create_task(session.get(f"http://localhost:{1000 + currWorker}/", json = {"cli": index, "codes": element})))
        ...
        return web.json_response({"erroe": e}, status=500)
